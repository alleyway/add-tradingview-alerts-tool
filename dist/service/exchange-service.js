...
const fetchByBit = async (quoteAsset) => {
    ...
    for (const symbol of symbols) {
        if (symbol.status === "Trading" && (quoteAsset === CONST_ALL || symbol.quote_currency === quoteAsset.toUpperCase())) {
            exchangeSymbols.push(new ExchangeSymbol("BYBIT", symbol.base_currency, symbol.quote_currency, `BYBIT:${symbol.alias}`));
        }
    }
...
const fetchKucoin = async (quoteAsset) => {
    ...
    for (const symObj of symbolArray) { // key = "AAVEAUD"
        ...
        if (symObj.enableTrading && (quoteAsset === CONST_ALL || symbolQuote === quoteAsset.toUpperCase())) {
            exchangeSymbols.push(new ExchangeSymbol("KUCOIN", symbolBase, symbolQuote));
        }
    }
    return exchangeSymbols;
};
    ...
    for (const key of Object.keys(symbolsObject)) { // key = "AAVEAUD"
        ...
        if (symbolObj.wsname) {
            ...
            if ((quoteAsset === CONST_ALL || symbolQuote === quoteAsset.toUpperCase())) {
                exchangeSymbols.push(new ExchangeSymbol("KRAKEN", symbolBase, symbolQuote));
            }
        }
        else {
