...
const fetchAndTransform = async (url, responsePath, transformer) => {
    ...
    for (const obj of resultsArray) {
        const baseSymbol = transformer(obj);
        if (baseSymbol)
...
export const fetchKraken = async () => {
    ...
    for (const key of keys) { // key = "AAVEAUD"
        ...
        baseSymbols.push(new BaseSymbol("KRAKEN", symbolBase, symbolQuote));
    }
    log.info(`returning ${baseSymbols.length} results symbols parsed`);
    return baseSymbols;
};
...
export const fetchBinanceFutures = async () => {
    const transformer = (obj) => {
        if (obj.status === "TRADING") {
            return new BaseSymbol("BINANCEFUTURES", obj.baseAsset, obj.quoteAsset, `BINANCE:${obj.baseAsset}${obj.quoteAsset}PERP`);
        }
        else {
