...
export const configureSingleAlertSettings = async (page, singleAlertSettings) => {
    ...
    const selectFromDropDown = async (conditionToMatch) => {
        ...
        let foundTextArray = Object.keys(matchingOptions).sort((a, b) => { return b.length - a.length; }); // put shortest text first so exact matches happen first
        //nothing found
        if (foundTextArray.length == 0)
